...
If you already run local inference, use the [runtime-neutral local GPU route](docs/local-gpu-routing.md) as a separately approved pilot. Do not reinstall LM Studio or Ollama automatically, nominate 512M/1B/2B models for general-purpose work, or trade a 27B model down to Q1. Start an average PC/laptop general-purpose pilot at the documented minimum-good specification and verify memory, tools and the complete task.

If local GPU, a free model or OmniRoute is enabled, install the [forced task-routing policy](docs/task-routing.md) in the host's supported pre-dispatch path. Every model-bound task must receive a route decision. A host that cannot enforce the decision must keep automatic add-on routing disabled.
