...
from validation import validate_api_cost_scenario, validate_platform_coverage, validate_auth_matrix, load_json_within, normalize_usage, validate_addon_inventory, validate_assets, validate_content, validate_links, validate_market, validate_routes
...
def main():
    ...
    findings = validate_links(root) + validate_routes(root) + validate_content(root) + validate_market(root) + validate_addon_inventory(root) + validate_assets(root) + validate_platform_coverage(root) + validate_auth_matrix(root) + validate_api_cost_scenario(root)
    examples_checked = 0
    try:
