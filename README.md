...
<p align="center">
  <a href="#api-cost-planning-scenario"><img src="assets/banner.png" width="1200" alt="Token Saving Blueprints. API-cost planning scenario: 38% Conservative, 53% Aggressive, and sometimes up to 75% with an eligible local GPU route."></a>
</p>
...
<p align="center">
  ...
  <a href="VALIDATION.md"><img src="https://img.shields.io/badge/checks-87%20local-44cc11?style=plastic" width="111" height="20" alt="Checks: 87 local tests"></a>
...
[Architecture](docs/architecture.md) · [Platform compatibility](docs/platform-coverage.md) · [Start here](START-HERE.md) · [Conservative](docs/modes/conservative.md) · [Aggressive](docs/modes/aggressive.md) · [Choose your harness](docs/harnesses/index.md) · [Evidence](docs/evidence.md)
...
- A [runtime-neutral local GPU route](docs/local-gpu-routing.md) for existing LM Studio, Ollama or other approved endpoints, with specification floors and fail-closed gates rather than model-product endorsements.
- A [forced task-routing layer](docs/task-routing.md) whose policy supplies canonical configurable precedence, route bounds and verifier authorization while code and schemas enforce the immutable v2 safety contract.
...
The highlighted **38% Conservative** and **53% Aggressive** scenarios include both add-ons. For workloads with a high share of tasks that pass the local quality and capability gates, modelled variable API-cost savings can **sometimes reach 75%**. This is an upper planning case, not the expected result or a token-reduction claim. The gains are not independent percentages to add together.
...
- Local GPU upper case: after Aggressive mode and OmniRoute, assume **65.5% of the remaining workload** passes the local route's quality and capability gates, local compute costs **5% of the replaced cloud inference**, and coordination/fallback overhead adds **5% of remaining spend**. The multiplier is `0.345 + 0.655 × 0.05 + 0.05 = 0.42775`, producing **$2,502.34**, or **75% lower variable API cost** after whole-percentage rounding.
...
For both cloud add-ons, that produces **$6,156** for Conservative and **$4,738.50** for Aggressive. The separate local GPU upper case produces **$2,502.34**. The table and upper case round reductions to whole percentages. Inputs and outputs are in [the scenario data](examples/api-cost-scenario.json); the repository checker recomputes them using decimal arithmetic.
...
## Optional delegation, local GPU and OmniRoute

Enabling any of these add-ons also enables the [forced task-routing policy](docs/task-routing.md). Every model-bound task must be classified and routed before dispatch. The shipped policy orders deterministic code, verified local, verified free and main, and a valid policy precedence change changes reducer output without Python edits. Eligibility requires fresh, fingerprint-bound evidence for the exact task class, tools, context, output, data and verifier method; coarse local-GPU admission alone never qualifies a task. Unknown or failed gates stay on main. If the host cannot enforce this decision, automatic add-on routing remains off.
...
If you already run local inference, the [runtime-neutral local GPU route](docs/local-gpu-routing.md) provides specification-led eligibility, memory, capability, verification and rollback gates. It does not install LM Studio/Ollama, download weights, endorse model products or treat local compute as inherent token savings. Its general-purpose floor rejects 512M/1B/2B routes and below-4-bit quantization, including Q1 at 27B.
