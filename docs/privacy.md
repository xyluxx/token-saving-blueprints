...
Check default telemetry, external model calls, automatic weight downloads, HTTP authentication, sandboxing and uninstall behavior. A local subprocess is not automatically a sandbox. A local proxy may still send data upstream or install a trust certificate. A local inference server should bind to loopback by default; exposing it to a LAN or public interface requires explicit authentication, firewall and retention review. Model files also require provenance, license and checksum review.
