...
### Forced add-on routing

When a local, free-model or OmniRoute lane is enabled, merge the normative [forced task-routing policy](../task-routing.md) into the existing project instructions and use a supported pre-dispatch hook, wrapper or supervisor for the executable decision. The control must build the explicit task-route record, call the deterministic policy, and allow only the returned route. Codex may populate known task fields, but uncertainty becomes `unknown` and stays on the main model. Project instructions alone are guidance, not a guaranteed enforcement boundary.

Before enabling automatic routing, prove that credentials, irreversible actions, high-reasoning work, missing verification, insufficient context and unverified workers remain on the main model. If the installed Codex version cannot intercept the actual subagent or alternate-provider dispatch path, keep automatic add-on routing disabled and use reviewed manual routing.
