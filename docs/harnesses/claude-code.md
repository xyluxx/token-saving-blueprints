...
When a local, free-model or OmniRoute lane is enabled, merge the normative [forced task-routing policy](../task-routing.md) into the existing project instructions and use a supported pre-dispatch hook or wrapper for the executable decision. The hook must build the explicit task-route record, call the deterministic policy, and allow only the returned route. Claude's natural-language judgment may populate known fields, but uncertainty is recorded as `unknown` and therefore remains on the main model. A prompt alone is not forced enforcement.

Verify negative cases before enabling automatic routing: credentials, irreversible actions, high-reasoning work, missing verification, insufficient context and an unverified worker must all remain on the main model. If the installed Claude Code version cannot enforce the decision at the actual delegation seam, keep automatic add-on routing disabled and use reviewed manual routing.
