...
* Standalone CLI: `cn` has its own TUI, headless execution, authentication and context controls. Its quickstart documents a direct Anthropic API-key option as well as hosted login. Do not presume every editor `config.yaml` provider or setting works identically in the frozen CLI.[20]
