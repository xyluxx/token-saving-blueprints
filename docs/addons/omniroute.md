...
[Start here](../../START-HERE.md) · [Delegation](../delegation.md) · [Forced task routing](../task-routing.md) · [Compression inventory](omniroute-compression.md) · [Free-provider choices](free-provider-lanes.md)

Enabling OmniRoute requires the [forced task-routing policy](../task-routing.md) before every model-bound dispatch, even when no local or free worker is enabled. The policy can retain the main model, use deterministic code, or select a separately verified local/free route. If the host cannot enforce that pre-dispatch decision, automatic OmniRoute routing remains disabled.
