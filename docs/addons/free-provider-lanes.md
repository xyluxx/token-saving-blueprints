...
Enabling this lane requires the [forced task-routing policy](../task-routing.md). The default free route accepts only bounded, low-reasoning, public-data work with no external effect, a deterministic verifier, a task-class trial and a verified upstream hard spending stop. Every other task stays on the main route unless it independently qualifies for an enabled local route.
