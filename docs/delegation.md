...
Once a local or free worker lane is enabled, the [forced task-routing layer](task-routing.md) is required for every model-bound task. The layer selects deterministic code, a verified local route, a verified free route or the main model from explicit task metadata. A worker cannot self-select work around the gate, and a failed verifier returns the task to the approved main fallback once.
