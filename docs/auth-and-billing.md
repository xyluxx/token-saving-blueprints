...
A local endpoint is its own provider route even when it exposes a familiar protocol. It does not inherit a cloud model's identity, entitlement, tool behavior or quality. If LM Studio, Ollama or another approved runtime is already present, use the [runtime-neutral local GPU guide](local-gpu-routing.md): preserve the current route as baseline, choose by specification, validate memory/tools/context, and keep fallback spending explicit. Local compute cost and API tokens avoided are separate accounting categories.
